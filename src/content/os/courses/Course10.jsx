    ...
    <>
              ...
              <Box type="definition">
                ...
                <ol className="list-decimal pl-5 text-sm space-y-1">
                  <li>{t('What is a signal — software interrupt concept', 'Ce este un signal — conceptul de întrerupere software')}</li>
                  <li>{t('Signal categories: errors, external events, explicit requests', 'Categorii de signals: erori, evenimente externe, cereri explicite')}</li>
                  <li>{t('Synchronous vs asynchronous signals', 'Signals sincrone vs. asincrone')}</li>
                  <li>{t('Generating signals: kill(), raise()', 'Generarea signal-elor: kill(), raise()')}</li>
                  <li>{t('Predefined signal types (SIGINT, SIGKILL, SIGSEGV, etc.)', 'Tipuri predefinite de signals (SIGINT, SIGKILL, SIGSEGV, etc.)')}</li>
                  <li>{t('Signal handling: default, ignore, custom handler', 'Tratarea signal-elor: implicit, ignorare, handler personalizat')}</li>
                  <li>{t('signal() — configuring handlers', 'signal() — configurarea signal handler-elor')}</li>
                  <li>{t('Writing safe signal handlers', 'Scrierea signal handler-elor sigure')}</li>
                  <li>{t('Blocking signals: sigprocmask(), sigpending()', 'Blocarea signal-elor: sigprocmask(), sigpending()')}</li>
                  <li>{t('Waiting for signals: pause(), sigsuspend()', 'Așteptarea signal-elor: pause(), sigsuspend()')}</li>
                </ol>
              </Box>
              ...
              <Section title={t('1. What is a Signal?', '1. Ce este un signal?')} id="course_10-what" checked={!!checked['course_10-what']} onCheck={() => toggleCheck('course_10-what')}>
                ...
                <svg viewBox="0 0 450 140" className="w-full max-w-lg mx-auto my-4" style={{fontFamily:'monospace',fontSize:10}}>
                  ...
                  <text x="220" y="72" textAnchor="middle" fill="#f59e0b" fontSize="9" fontWeight="bold">{t('Signal', 'Signal')}</text>
...
$ killall -SIGINT myprg # send SIGINT to all processes named "myprg"`}</Code>
                ...
                <Box type="definition">
                  <p className="font-bold">{t('Common keyboard signals:', 'Signals comune de la tastatură:')}</p>
              ...
              <Section title={t('3. Signal Types', '3. Tipuri de signals')} id="course_10-types" checked={!!checked['course_10-types']} onCheck={() => toggleCheck('course_10-types')}>
                <Box type="formula">
                  ...
                  <table className="text-sm mt-1 w-full">
                    <thead><tr className="border-b dark:border-gray-600"><th className="p-1">{t('Signal', 'Signal')}</th><th className="p-1">{t('Default', 'Implicit')}</th><th className="p-1">{t('Meaning', 'Semnificație')}</th></tr></thead>
                    <tbody>
                      ...
                      <tr className="border-b dark:border-gray-700"><td className="p-1 font-mono">SIGPIPE (13)</td><td className="p-1">{t('Terminate', 'Terminare')}</td><td className="p-1">{t('Write to pipe with no reader', 'Scriere pe pipe fără cititor')}</td></tr>
                ...
                <Box type="warning">
                  <p className="font-bold">{t('Uncatchable signals:', 'Signals care nu pot fi interceptate:')}</p>
              ...
              <Section title={t('4. Signal Handling with signal()', '4. Tratarea signal-elor cu signal()')} id="course_10-handle" checked={!!checked['course_10-handle']} onCheck={() => toggleCheck('course_10-handle')}>
                ...
                <Box type="definition">
                  <ol className="list-decimal pl-5 text-sm">
                    ...
                    <li><strong>{t('Ignore', 'Ignorare')}</strong>{t(' (SIG_IGN) — signal is discarded', ' (SIG_IGN) — signal-ul este ignorat')}</li>
    ...
    if (count >= 3) {
        ...
        signal(sig, SIG_DFL);  // next signal uses default action
              ...
              <Section title={t('5. Blocking Signals', '5. Blocarea signal-elor')} id="course_10-block" checked={!!checked['course_10-block']} onCheck={() => toggleCheck('course_10-block')}>
                ...
                <Box type="formula">
                  <p className="font-bold">{t('sigprocmask — block/unblock signal types:', 'sigprocmask — blocarea/deblocarea tipurilor de signals:')}</p>
...
if (sigismember(&pending, SIGINT))
    printf("SIGINT is waiting to be delivered\\n");`}</Code>
                ...
                <Box type="theorem">
                  ...
                  <p>{t('Block signals during critical code sections where interruption would corrupt state. Don\'t ignore — you\'d lose the signal permanently. Blocking defers delivery; ignoring discards it.', 'Blocați signal-ele în secțiunile critice de cod unde o întrerupere ar corupe starea. Nu ignorați — ați pierde signal-ul permanent. Blocarea amână livrarea; ignorarea îl elimină.')}</p>
                </Box>
              </Section>
...
sigsuspend(&wait_mask);           // sleep until SIGQUIT arrives`}</Code>
              ...
              <Section title={t('Cheat Sheet', 'Rezumat rapid')} id="course_10-cheat" checked={!!checked['course_10-cheat']} onCheck={() => toggleCheck('course_10-cheat')}>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm font-mono">
                  ...
                  <Box type="formula"><p className="font-bold">{t('Block', 'Blocare')}</p><p>sigprocmask(SIG_BLOCK/UNBLOCK/SET)</p><p>sigset_t, sigemptyset, sigaddset</p><p>{t('sigpending — check queued signals', 'sigpending — verificare signals în coadă')}</p></Box>
                  <Box type="formula"><p className="font-bold">{t('Wait', 'Așteptare')}</p><p>{t('pause() — any signal', 'pause() — orice signal')}</p><p>{t('sigsuspend(mask) — atomic, safe', 'sigsuspend(mask) — atomic, sigur')}</p><p>{t('SIGKILL/SIGSTOP: uncatchable!', 'SIGKILL/SIGSTOP: nu pot fi interceptate!')}</p></Box>
                </div>
              </Section>
              ...
              <Section title={t('Self-Test (10 Questions)', 'Autoevaluare (10 întrebări)')} id="course_10-quiz" checked={!!checked['course_10-quiz']} onCheck={() => toggleCheck('course_10-quiz')}>
                ...
                <Toggle question={t('4. Why is printf() unsafe inside a signal handler?', '4. De ce printf() este nesigur în interiorul unui signal handler?')} answer={t('printf() is NOT async-signal-safe. It uses internal buffers and locks. If the signal interrupts printf() in the main program, calling printf() in the handler can deadlock or corrupt the buffer. Use write() instead, or set a flag.', 'printf() NU este sigur pentru signals asincrone. Folosește buffere interne și lacăte. Dacă signal-ul întrerupe printf() în programul principal, apelarea printf() în handler poate provoca deadlock sau coruperea buffer-ului. Utilizați write() în schimb sau setați un indicator.')} hideLabel={t('Hide', 'Ascunde')} showLabel={t('Show Answer', 'Arată răspunsul')} />
                <Toggle question={t('5. What is the difference between blocking and ignoring a signal?', '5. Care este diferența dintre blocarea și ignorarea unui signal?')} answer={t('Ignoring (SIG_IGN): signal is permanently discarded. Blocking (sigprocmask): signal is QUEUED and delivered when unblocked. Blocking preserves the signal; ignoring loses it.', 'Ignorare (SIG_IGN): signal-ul este eliminat permanent. Blocare (sigprocmask): signal-ul este PUS ÎN COADĂ și livrat când este deblocat. Blocarea păstrează signal-ul; ignorarea îl pierde.')} hideLabel={t('Hide', 'Ascunde')} showLabel={t('Show Answer', 'Arată răspunsul')} />
                <Toggle question={t('6. What exit status does a process killed by signal N have?', '6. Ce cod de ieșire are un proces terminat de signal-ul N?')} answer={t('128 + N. For example, SIGKILL (9) → exit status 137. SIGSEGV (11) → exit status 139. You can check with WIFSIGNALED and WTERMSIG macros from wait().', '128 + N. De exemplu, SIGKILL (9) → cod de ieșire 137. SIGSEGV (11) → cod de ieșire 139. Puteți verifica cu macro-urile WIFSIGNALED și WTERMSIG din wait().')} hideLabel={t('Hide', 'Ascunde')} showLabel={t('Show Answer', 'Arată răspunsul')} />
                <Toggle question={t('7. Why is sigsuspend() preferred over pause()?', '7. De ce sigsuspend() este preferat față de pause()?')} answer={t('pause() has a race condition: the signal may arrive between sigprocmask(unblock) and pause(), causing pause to block forever. sigsuspend() atomically changes the mask AND suspends, eliminating the race.', 'pause() are o condiție de cursă: signal-ul poate sosi între sigprocmask(unblock) și pause(), cauzând blocarea la infinit a lui pause. sigsuspend() schimbă atomic masca ȘI suspendă, eliminând condiția de cursă.')} hideLabel={t('Hide', 'Ascunde')} showLabel={t('Show Answer', 'Arată răspunsul')} />
                <Toggle question={t('8. What does kill(pid, 0) do?', '8. Ce face kill(pid, 0)?')} answer={t("It doesn't send any signal. It tests whether the process with the given PID exists and whether you have permission to send it signals. Returns 0 if yes, -1 if not. Useful for checking if a process is alive.", 'Nu trimite niciun signal. Testează dacă procesul cu PID-ul dat există și dacă aveți permisiunea de a-i trimite signals. Returnează 0 dacă da, -1 dacă nu. Util pentru a verifica dacă un proces este în viață.')} hideLabel={t('Hide', 'Ascunde')} showLabel={t('Show Answer', 'Arată răspunsul')} />
                <Toggle question={t('9. What signal is sent when you write to a pipe with no readers?', '9. Ce signal este trimis când scrieți pe un pipe fără cititori?')} answer={t('SIGPIPE (13). Default action: terminate the process. This connects to Course 9 — it\'s why closing unused pipe ends matters. You can catch SIGPIPE to handle the situation gracefully instead of dying.', 'SIGPIPE (13). Acțiunea implicită: terminarea procesului. Aceasta se leagă de Cursul 9 — de aceea contează închiderea capetelor neutilizate ale pipe-ului. Puteți intercepta SIGPIPE pentru a gestiona situația gracios în loc să terminați.')} hideLabel={t('Hide', 'Ascunde')} showLabel={t('Show Answer', 'Arată răspunsul')} />
                <Toggle question={t('10. After exec(), what happens to signal handlers?', '10. După exec(), ce se întâmplă cu signal handler-ele?')} answer={t('Custom handlers are RESET to SIG_DFL (because the handler code no longer exists in the new program). Signals set to SIG_IGN remain ignored. The signal mask (blocked signals) is preserved.', 'Handler-ele personalizate sunt RESETATE la SIG_DFL (deoarece codul handler-ului nu mai există în noul program). Signal-ele setate la SIG_IGN rămân ignorate. Masca de signals (signals blocate) este păstrată.')} hideLabel={t('Hide', 'Ascunde')} showLabel={t('Show Answer', 'Arată răspunsul')} />
              </Section>
    </>
  );
}
