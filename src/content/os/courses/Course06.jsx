    ...
    <>
              ...
              <Box type="definition">
                ...
                <ol className="list-decimal pl-5 text-sm space-y-1">
                  ...
                  <li>{t('Termination: exit (normal) vs abort/signal (abnormal)', 'Terminare: exit (normală) vs. abort/signal (anormală)')}</li>
...
nanosleep(&req, &rem); // nanosecond precision, restartable`}</Code>
                ...
                <Box type="warning">
                  ...
                  <p>{t('Actual sleep duration may ', 'Durata efectivă de suspendare poate ')}<strong>{t('exceed', 'depăși')}</strong>{t(' the requested time (OS scheduling). It may also be ', ' valoarea specificată (planificarea SO). Poate fi și ')}<strong>{t('shorter', 'mai scurtă')}</strong>{t(' if a signal interrupts the sleep (errno = EINTR). Only ', ' dacă un signal întrerupe suspendarea (errno = EINTR). Doar ')} <code>nanosleep</code> {t('reports the remaining time via its second argument.', 'raportează timpul rămas prin al doilea argument.')}</p>
                </Box>
