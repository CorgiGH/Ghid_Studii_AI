...
const geometryQuestions = [
  {
    ...
    options: [
      { text: { en: 'P, Q, R are collinear', ro: 'P, Q, R sunt coliniare' }, correct: false, feedback: { en: 'Collinear means ccw = 0, not > 0.', ro: 'Coliniar înseamnă ccw = 0, nu > 0.' } },
      { text: { en: 'The turn from P→Q→R is counterclockwise', ro: 'Virajul P→Q→R este în sens invers acelor de ceasornic' }, correct: true },
      { text: { en: 'The turn from P→Q→R is clockwise', ro: 'Virajul P→Q→R este în sensul acelor de ceasornic' }, correct: false, feedback: { en: 'Clockwise corresponds to ccw < 0.', ro: 'Sens orar corespunde lui ccw < 0.' } },
      { text: { en: 'Q is closer to P than R', ro: 'Q este mai aproape de P decât R' }, correct: false, feedback: { en: 'ccw is the sign of the cross product — unrelated to distances.', ro: 'ccw e semnul produsului vectorial — fără legătură cu distanțele.' } },
    ],
  ...
  },
  {
    ...
    options: [
      { text: { en: 'When ccw of three consecutive points is > 0', ro: 'Când ccw a trei puncte consecutive este > 0' }, correct: false, feedback: { en: 'ccw > 0 (left turn) keeps the point; we eliminate on right turns.', ro: 'ccw > 0 (viraj la stânga) păstrează punctul; eliminăm la viraje la dreapta.' } },
      ...
      { text: { en: 'When the point is the farthest from center', ro: 'Când punctul este cel mai depărtat de centru' }, correct: false, feedback: { en: 'Distance from center is irrelevant to convexity.', ro: 'Distanța față de centru e irelevantă pentru convexitate.' } },
      { text: { en: 'When the point has the smallest polar angle', ro: 'Când punctul are cel mai mic unghi polar' }, correct: false, feedback: { en: 'Smallest polar angle is used to choose the starting anchor, not eliminate.', ro: 'Cel mai mic unghi polar e folosit pentru alegerea ancorei de start, nu pentru eliminare.' } },
    ],
  ...
  },
  {
    ...
    options: [
      { text: 'O(n)', correct: false, feedback: { en: 'Sorting by polar angle alone is Ω(n log n).', ro: 'Sortarea după unghi polar singură e Ω(n log n).' } },
      { text: 'O(n log n)', correct: true },
      { text: 'O(n²)', correct: false, feedback: { en: 'Too loose; the scan phase is O(n) once sorted.', ro: 'Prea puțin strâns; faza de scanare e O(n) după sortare.' } },
      { text: 'O(n³)', correct: false, feedback: { en: 'Much too high; the naive cubic algorithm enumerates triples, not Graham.', ro: 'Mult prea mult; algoritmul cubic naiv enumeră triplete, nu Graham.' } },
    ],
  ...
  },
  {
    ...
    options: [
      { text: 'O(n log n)', correct: false, feedback: { en: 'With already-sorted input, the sorting step can be skipped; only the scan remains.', ro: 'Cu input deja sortat, pasul de sortare poate fi sărit; rămâne doar scanarea.' } },
      { text: 'O(n)', correct: true },
      { text: 'O(n²)', correct: false, feedback: { en: 'No quadratic work here — scanning with a stack is strictly linear.', ro: 'Fără lucru pătratic aici — scanarea cu stivă e strict liniară.' } },
      { text: 'O(n log² n)', correct: false, feedback: { en: 'Log² arises from complicated nested sorts; CHLPS just does one linear pass.', ro: 'Log² apare la sortări imbricate complicate; CHLPS face doar o parcurgere liniară.' } },
    ],
  ...
  },
  {
    ...
    options: [
      { text: { en: 'Check if P is above all edges', ro: 'Verificăm dacă P este deasupra tuturor muchiilor' }, correct: false, feedback: { en: '"Above" is ill-defined for arbitrary polygons; use ray casting instead.', ro: '„Deasupra" e nedefinit pentru poligoane oarecare; folosește ray casting.' } },
      ...
      { text: { en: 'Compute distance from P to all vertices', ro: 'Calculăm distanța de la P la toate vârfurile' }, correct: false, feedback: { en: 'Distance tells nothing about inside/outside for arbitrary shapes.', ro: 'Distanța nu spune nimic despre interior/exterior pentru forme oarecare.' } },
      { text: { en: 'Check if ccw(P, v₁, v₂) > 0 for all edges', ro: 'Verificăm dacă ccw(P, v₁, v₂) > 0 pentru toate muchiile' }, correct: false, feedback: { en: 'That works only for *convex* polygons, not arbitrary simple polylines.', ro: 'Funcționează doar pentru poligoane *convexe*, nu pentru linii poligonale simple oarecare.' } },
    ],
  ...
  },
  {
    ...
    options: [
      { text: { en: 'All pairs of points', ro: 'Toate perechile de puncte' }, correct: false, feedback: { en: 'Correct but O(n²); rotating calipers on the hull is O(n log n).', ro: 'Corect dar O(n²); rotating calipers pe hull e O(n log n).' } },
      ...
      { text: { en: 'The center of gravity', ro: 'Centrul de greutate' }, correct: false, feedback: { en: 'Center of gravity is used elsewhere (e.g. as Graham-scan origin), not for the diameter.', ro: 'Centrul de greutate se folosește altundeva (ex. origine Graham), nu pentru diametru.' } },
      { text: { en: 'Voronoi diagram', ro: 'Diagrama Voronoi' }, correct: false, feedback: { en: 'Voronoi computes nearest neighbours, not farthest pairs.', ro: 'Voronoi calculează vecinii cei mai apropiați, nu perechile cele mai depărtate.' } },
    ],
  ...
  },
  {
    ...
    options: [
      { text: { en: 'A triangulation of S', ro: 'O triangulare a lui S' }, correct: false, feedback: { en: 'That is the *Delaunay* triangulation, which is the dual of Voronoi — related but different.', ro: 'Aceea e triangulația *Delaunay*, dualul Voronoi — înrudite dar diferite.' } },
      ...
      { text: { en: 'The convex hull of S', ro: 'Înfășurătoarea convexă a lui S' }, correct: false, feedback: { en: 'Convex hull is a polygon; Voronoi is a plane partition into polygons.', ro: 'Hull-ul e un poligon; Voronoi e o partiție a planului în poligoane.' } },
      { text: { en: 'The minimum spanning tree of S', ro: 'Arborele de acoperire minim al lui S' }, correct: false, feedback: { en: 'MST is a graph structure; Voronoi is a plane decomposition.', ro: 'MST e o structură de graf; Voronoi e o descompunere a planului.' } },
    ],
  ...
  },
  {
    ...
    options: [
      { text: 'O(n)', correct: false, feedback: { en: 'The divide-and-conquer merge step is O(n), but sorting/recursion makes the overall bound O(n log n).', ro: 'Pasul de merge e O(n), dar sortarea/recursia fac marginea globală O(n log n).' } },
      { text: 'O(n log n)', correct: true },
      { text: 'O(n²)', correct: false, feedback: { en: 'That is an incremental (non-D&C) approach; D&C is log-linear.', ro: 'Aceea e abordare incrementală; D&C e log-liniar.' } },
      { text: 'O(n² log n)', correct: false, feedback: { en: 'Way too high — even the simplest quadratic algorithm is faster.', ro: 'Mult prea mult — până și algoritmul pătratic simplu e mai rapid.' } },
    ],
  ...
  },
  {
    ...
    options: [
      { text: { en: 'Turing reduction (multiple oracle calls)', ro: 'Reducere Turing (apeluri multiple ale oracolului)' }, correct: false, feedback: { en: 'Only one oracle call is needed (single ALL NEAREST NEIGHBOR query).', ro: 'E nevoie de un singur apel oracol (o singură interogare ALL NEAREST NEIGHBOR).' } },
      { text: { en: 'Karp reduction (single oracle call)', ro: 'Reducere Karp (un singur apel al oracolului)' }, correct: true },
      { text: { en: 'Cook reduction', ro: 'Reducere Cook' }, correct: false, feedback: { en: 'Cook = Turing for decision problems; here we use the stricter Karp form.', ro: 'Cook = Turing pentru probleme de decizie; aici e forma mai strictă Karp.' } },
      { text: { en: 'Not a valid reduction', ro: 'Nu este o reducere validă' }, correct: false, feedback: { en: 'It is a valid reduction — the answer is embedded as an entry in the returned list.', ro: 'E reducere validă — răspunsul e o poziție din lista returnată.' } },
    ],
  ...
  },
  {
    ...
    options: [
      { text: { en: 'As the starting point of the scan', ro: 'Ca punct de start al scanării' }, correct: false, feedback: { en: 'The scan starts from the point with the lowest y-coordinate, not the centroid.', ro: 'Scanarea pleacă din punctul cu ordonata minimă, nu din centroid.' } },
      { text: { en: 'As the origin for polar coordinate sorting', ro: 'Ca origine pentru sortarea după coordonate polare' }, correct: true },
      { text: { en: 'To determine the convex hull diameter', ro: 'Pentru a determina diametrul înfășurătorii convexe' }, correct: false, feedback: { en: 'Diameter uses antipodal pairs on the hull, not the centroid.', ro: 'Diametrul folosește perechi antipodale pe hull, nu centroidul.' } },
      { text: { en: 'To check if a point is inside the hull', ro: 'Pentru a verifica dacă un punct este în interiorul înfășurătorii' }, correct: false, feedback: { en: 'Inside-hull test uses ray casting or ccw checks; centroid is unrelated here.', ro: 'Testul de interior folosește ray casting sau ccw; centroidul nu e implicat.' } },
    ],
...
const npQuestions = [
  {
    ...
    options: [
      ...
      { text: { en: 'False', ro: 'Fals' }, correct: false, feedback: { en: 'If an NP-complete problem were in P, all of NP would collapse to P — contradicting P ≠ NP.', ro: 'Dacă o problemă NP-completă ar fi în P, tot NP-ul ar colapsa la P — contradicție cu P ≠ NP.' } },
    ],
  ...
  },
  {
    ...
    options: [
      { text: { en: 'Yes', ro: 'Da' }, correct: false, feedback: { en: 'Would force P = NP; P ⊂ NP allows some NP problems in P without all of them being polynomial.', ro: 'Ar forța P = NP; P ⊂ NP admite doar unele probleme NP în P, nu toate.' } },
  ...
  },
  {
    ...
    options: [
      { text: { en: 'Decision problem + deterministic + polynomial', ro: 'Problemă de decizie + determinist + polinomial' }, correct: false, feedback: { en: 'That defines P, not NP. NP uses nondeterministic.', ro: 'Aceea definește P, nu NP. NP folosește nedeterminism.' } },
      ...
      { text: { en: 'Optimization problem + nondeterministic + exponential', ro: 'Problemă de optimizare + nedeterminist + exponențial' }, correct: false, feedback: { en: 'Wrong on two counts: NP is for decision problems, and requires polynomial time.', ro: 'Greșit pe două părți: NP e pentru decizie și cere timp polinomial.' } },
      { text: { en: 'Decision problem + probabilistic + polynomial', ro: 'Problemă de decizie + probabilist + polinomial' }, correct: false, feedback: { en: 'That defines BPP, not NP.', ro: 'Aceea definește BPP, nu NP.' } },
    ],
  ...
  },
  {
    ...
    options: [
      ...
      { text: { en: 'Every edge has at least one endpoint in V\'', ro: 'Fiecare muchie are cel puțin un capăt în V\'' }, correct: false, feedback: { en: 'That describes VERTEX COVER, not INDEPENDENT-SET.', ro: 'Aceea descrie VERTEX COVER, nu INDEPENDENT-SET.' } },
      { text: { en: 'All selected nodes form a complete subgraph', ro: 'Toate nodurile selectate formează un subgraf complet' }, correct: false, feedback: { en: 'That describes CLIQUE, the exact opposite.', ro: 'Aceea descrie CLIQUE, exact opusul.' } },
      { text: { en: 'V\' contains all nodes of degree ≥ k', ro: 'V\' conține toate nodurile de grad ≥ k' }, correct: false, feedback: { en: 'Degree has no role in the independent-set definition.', ro: 'Gradul nu are rol în definiția mulțimii independente.' } },
    ],
  ...
  },
  {
    ...
    options: [
      { text: { en: 'VERTEX COVER → INDEPENDENT-SET', ro: 'VERTEX COVER → INDEPENDENT-SET' }, correct: false, feedback: { en: 'Reverse direction: shows VC is no harder than IS, not that VC is hard.', ro: 'Direcție inversă: arată că VC nu e mai grea ca IS, nu că VC e grea.' } },
      { text: { en: 'INDEPENDENT-SET → VERTEX COVER', ro: 'INDEPENDENT-SET → VERTEX COVER' }, correct: true },
      { text: { en: 'Either direction works', ro: 'Oricare direcție funcționează' }, correct: false, feedback: { en: 'Direction matters in hardness proofs; one direction proves hardness, the other proves ease.', ro: 'Direcția contează în demonstrațiile de dificultate; o direcție demonstrează dificultatea, cealaltă ușurința.' } },
      { text: { en: 'Neither, they must both reduce to SAT', ro: 'Niciuna, ambele trebuie să se reducă la SAT' }, correct: false, feedback: { en: 'Reducing to SAT proves membership in NP, not hardness.', ro: 'Reducerea la SAT dovedește apartenența la NP, nu dificultatea.' } },
    ],
  ...
  },
  {
    ...
    options: [
      ...
      { text: { en: 'Remove all edges, set k = |V| - k', ro: 'Se șterg toate muchiile, se setează k = |V| - k' }, correct: false, feedback: { en: 'Deleting all edges trivializes the problem; we need to invert adjacency.', ro: 'Ștergerea tuturor muchiilor banalizează problema; trebuie inversată adiacența.' } },
      { text: { en: 'Add edges between non-adjacent vertices, keep k', ro: 'Se adaugă muchii între vârfurile neadiacente, se păstrează k' }, correct: false, feedback: { en: 'You also need to *remove* the original edges — the full complement operation does both.', ro: 'Trebuie să *ștergi* și muchiile originale — operația de complement face ambele.' } },
      { text: { en: 'Double all edges, set k = 2k', ro: 'Se dublează toate muchiile, se setează k = 2k' }, correct: false, feedback: { en: 'Doubling edges does not create new non-adjacency; unrelated to the reduction.', ro: 'Dublarea muchiilor nu creează neadiacență; fără legătură cu reducerea.' } },
    ],
  ...
  },
  {
    ...
    options: [
      ...
      { text: { en: 'Complement graph, keep k', ro: 'Se complementează graful, se păstrează k' }, correct: false, feedback: { en: 'Complementing is for CLIQUE ↔ IS; VC and IS are complements *within the same graph*.', ro: 'Complementarea e pentru CLIQUE ↔ IS; VC și IS sunt complementare *în același graf*.' } },
      { text: { en: 'Same graph G, keep k', ro: 'Același graf G, se păstrează k' }, correct: false, feedback: { en: 'Must shift k: a VC of ≤ k gives an IS of ≥ |V|−k, not the same k.', ro: 'Trebuie modificat k: un VC ≤ k dă un IS ≥ |V|−k, nu același k.' } },
      { text: { en: 'Complement graph, change k to |V| - k', ro: 'Se complementează graful, se schimbă k în |V| - k' }, correct: false, feedback: { en: 'Over-transforms: only the k shift is needed; the graph stays the same.', ro: 'Transformă prea mult: doar decalarea lui k e necesară; graful rămâne același.' } },
    ],
  ...
  },
  {
    ...
    options: [
      { text: { en: 'It is in P and NP-hard', ro: 'Este în P și NP-dificilă' }, correct: false, feedback: { en: 'NP-hard + in P would imply P = NP; that is not required to prove NP-completeness.', ro: 'NP-dificilă + în P ar implica P = NP; nu e necesar pentru NP-completitudine.' } },
      { text: { en: 'It is in NP and NP-hard', ro: 'Este în NP și NP-dificilă' }, correct: true },
      { text: { en: 'It is in NP and has no polynomial algorithm', ro: 'Este în NP și nu are algoritm polinomial' }, correct: false, feedback: { en: 'No polynomial-time algorithm is an open question; proving NP-completeness does not require it.', ro: 'Lipsa algoritmului polinomial e întrebare deschisă; nu trebuie demonstrată pentru NP-completitudine.' } },
      { text: { en: 'It reduces to SAT', ro: 'Se reduce la SAT' }, correct: false, feedback: { en: 'Reducing TO SAT shows membership in NP, not NP-hardness; for hardness, reduce FROM a known hard problem.', ro: 'Reducerea LA SAT arată apartenența la NP, nu dificultatea; pentru dificultate, reduce DIN o problemă dificilă.' } },
    ],
  ...
  },
  {
    ...
    options: [
      { text: { en: '2-approximation', ro: 'Aproximare cu factor 2' }, correct: false, feedback: { en: 'SSD admits an FPTAS (arbitrary ε), so calling the approximation factor 2 is an understatement.', ro: 'SSD admite un FPTAS (ε arbitrar), deci un factor de 2 e prea conservator.' } },
      { text: { en: 'Fully polynomial-time approximation scheme (FPTAS)', ro: 'Schemă de aproximare complet polinomială (FPTAS)' }, correct: true },
      { text: { en: 'Constant-factor approximation', ro: 'Aproximare cu factor constant' }, correct: false, feedback: { en: 'FPTAS lets ε be arbitrarily small (any constant); that is stronger than a fixed constant factor.', ro: 'FPTAS permite ε arbitrar de mic; mai tare decât un factor constant fix.' } },
      { text: { en: 'No approximation exists', ro: 'Nu există aproximare' }, correct: false, feedback: { en: 'SSD has a well-known FPTAS; approximations do exist.', ro: 'SSD are un FPTAS binecunoscut; aproximările există.' } },
    ],
  ...
  },
  {
    ...
    options: [
      { text: '2', correct: false, feedback: { en: 'Constant-factor would be much stronger; greedy is only ln n-approximate in the worst case.', ro: 'Factor constant ar fi mult mai tare; greedy e doar ln n în cazul cel mai rău.' } },
      { text: 'log n', correct: false, feedback: { en: 'Close but not tight — the precise bound is the n-th harmonic Hₙ ≈ ln n + γ.', ro: 'Aproape dar nu exact — marginea precisă e Hₙ ≈ ln n + γ.' } },
      { text: 'Hₙ = 1 + 1/2 + ... + 1/n', correct: true },
      { text: 'n', correct: false, feedback: { en: 'Linear approximation is essentially useless; greedy does much better at Hₙ.', ro: 'Aproximare liniară e aproape inutilă; greedy obține mult mai bine, Hₙ.' } },
    ],
  ...
  },
  {
    ...
    options: [
      { text: { en: 'Yes, always', ro: 'Da, întotdeauna' }, correct: false, feedback: { en: 'Karp is restricted to decision problems; optimization needs Turing/Cook reductions.', ro: 'Karp e restrâns la probleme de decizie; optimizarea cere reduceri Turing/Cook.' } },
      ...
      { text: { en: 'Only if both problems are in NP', ro: 'Doar dacă ambele probleme sunt în NP' }, correct: false, feedback: { en: 'NP membership is orthogonal; Karp\'s restriction is about decision vs non-decision.', ro: 'Apartenența la NP e ortogonală; restricția Karp e între decizie și non-decizie.' } },
      { text: { en: 'Only for minimization problems', ro: 'Doar pentru probleme de minimizare' }, correct: false, feedback: { en: 'Optimization direction does not matter; the issue is that Karp\'s output is yes/no.', ro: 'Direcția optimizării nu contează; problema e că output-ul Karp e da/nu.' } },
    ],
  ...
  },
  {
    ...
    options: [
      { text: 'P = NP', correct: false, feedback: { en: 'Unproven — this is the famous open question.', ro: 'Nedemonstrat — e celebra întrebare deschisă.' } },
      { text: { en: 'P ⊆ NP (known), P = NP unknown', ro: 'P ⊆ NP (cunoscut), P = NP necunoscut' }, correct: true },
      { text: 'P ⊃ NP', correct: false, feedback: { en: 'Would mean NP ⊂ P, which is the strong form of P = NP — also unproven and widely disbelieved.', ro: 'Ar însemna NP ⊂ P, forma tare a lui P = NP — nedemonstrată și considerată falsă.' } },
      { text: { en: 'P and NP are disjoint', ro: 'P și NP sunt disjuncte' }, correct: false, feedback: { en: 'Clearly wrong — every problem in P is trivially in NP (deterministic is a special case of nondeterministic).', ro: 'Clar greșit — orice problemă din P e și în NP (determinismul e caz particular de nedeterminism).' } },
    ],
